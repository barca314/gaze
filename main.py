...
workers = 6
epochs = 25
batch_size = torch.cuda.device_count()*10 # Change if out of cuda memory
...
def main():
    ...
    wandb.init(project="Gaze_customized", entity="barca0314", config={"learning_rate": lr, "epochs": epochs, "batch_size": batch_size,"workers": workers, "momentum": momentum, "weight_decay": 1e-4})
...
def train(train_loader, model, criterion,optimizer, epoch):
    ...
    wandb.log({'epoch': epoch, 'train_data': data_time.avg, 'train_loss': losses.avg, 'batch_time': batch_time.avg})
...
def validate(val_loader, model, criterion, epoch):
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze) in enumerate(val_loader):
        ...
        imEyeR = imEyeR.cuda()
        faceGrid = faceGrid.cuda()
        #print("gaze before cuda:"+str(gaze.shape))
        gaze = gaze.cuda()
        #print("gaze cuda:"+str(gaze.shape))
        ...
        # print("imface:"+str(imFace.shape))
        # print("imeyel:"+str(imEyeL.shape))
        # print("imeyere:"+str(imEyeR.shape))
        # print("facegrid:"+str(faceGrid.shape))
        #print("gaze:"+str(gaze.shape))
    ...
    wandb.log({'epoch': epoch, 'val_loss': losses.avg, 'Error L2': lossesLin.avg})
    # with open('l2error_without_lrn.txt', 'r+',encoding='utf-8') as f:
    #     content = f.read()
    #     f.seek(0, 0)
    #     f.write('epoch['+str(epoch)+'] Error L2: '+str(lossesLin.avg)+'\n'+content)
    #     print('Error L2 saved to text')
